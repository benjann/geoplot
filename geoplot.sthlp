...
{phang}
    ...
    from {it:frame} even if there is a linked shape frame. Option {cmd:shp} has
...
{phang}
    ...
    red to blue HCL color scheme. The default is {cmd:color(Set1)} if
...
{phang2}
    ...
    {opt c:olor()} or {opt si:ze()}. Default settings are
